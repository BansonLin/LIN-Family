...
- **data/games.json schema**(2026-07-24 新增):`{id, name, risk: green|yellow|blue|red(情緒風險層), players, coop, assist[], motion, pause, rating, handheld, trip: 帶|不帶|看狀況, who:{big,lil}, note, evidence{url,label}}`。**遊戲規格屬產品事實,一律引任天堂官方/ESRB 官方頁,禁止用模型知識填寫;查不到標「待確認」**。台灣遊戲軟體分級查不到者改附 ESRB 並標明制度。
- **旅行帶機**:行程與每日建議寫在 switch-template.html 的 TRIP 常數(一次性行程,比照 disney.html);帶機規定(行動電源、電壓、機上使用、腕帶)逐條附官方出處,**規則變動頻繁,出發前需重查**。
