...
每筆回報格式:`{id, kind:"kids"|"adult", name, date, rating 1–5, note, ts}`。逐筆處理:
...
3. **寫入**:於該 venue 的 family_log 追加 `{date, 評分: rating, 心得: note, ts}`;adult 若無 family_log 欄位則先建 `[]`(大人 schema 補 family_log,與親子一致)。林家實測權重高於一切網路來源。
