...
**餐食版**:參數形如「{菜名} 姊{1–5} 弟{1–5} {備註}」(或明顯是菜名)→ 改在 data/recipes.json 依 name 找(找不到 → 停下回報,不得自行新增),於 family_log 追加 {date: 今日, 姊姊接受度, 弟弟接受度, 備註}(缺哪個孩子就省略該鍵);食譜庫沒有 last_verified,不更新。

完成即停止,輸出:✅ 已寫入的 venue/食譜與內容、下一步建議一行。
